...
public class AgentMojo extends AbstractJacocoMojo {
   ...
   * parameter a random identifier is created by the agent.
   * 
   ...
   * <ul>
   * <li>file: At VM termination execution data is written to the file specified
   * in the {@link #destfile}.</li>
   ...
   * connect to when the output method is tcpclient. In tcpserver mode the value
   * "*" causes the agent to accept connections on any local address.
   * 
  ...
  /**
   * Port to bind to when the output method is tcpserver or connect to when the
   * output method is tcpclient. In tcpserver mode the port must be available,
  ...
  private File getSnapshotAgentJarFile() {
    final Artifact jacocoAgentArtifact = pluginArtifactMap
        .get(SNAPSHOT_AGENT_ARTIFACT_NAME);
    return jacocoAgentArtifact.getFile();
  }
